...
def main(
    ...
    for m in tqdm(uni_structure_names, total=len(uni_structure_names)):
        ...
        # Get best model by val loss
        best_model = it_df.iloc[np.argmin(it_df['validation loss'].values.astype(np.float32))]
        sel_ckpt = best_model['summary_dirs']
        sel_df = it_df[it_df['summary_dirs'] == sel_ckpt]

        # Grab training/val loss curves for this model
        best_dfs += [best_model]
        dfs += [sorted_df]
        curves += [sel_df]
    ...
    dfs = pd.concat(dfs)
    best_dfs = pd.DataFrame(best_dfs)
    curves = pd.DataFrame(curves)
    curves.to_csv('curves_%s.csv' % experiment_name)
    ckpts = best_dfs['checkpoints'].values
    models = best_dfs['model names'].values
    datasets = best_dfs['datasets'].values

    cmds = []
    for ck, mo, da in zip(ckpts, models, datasets):
        ck_num = re.search('\d+', ck.split(os.path.sep)[-1]).group()
        ck = '%s-%s' % (ck, ck_num)
        cmds += ['CUDA_VISIBLE_DEVICES=6 python run_job.py --experiment=%s --model=%s --ckpt=%s --no_db --test --out_dir=%s' % (experiment_name, mo, ck, 'test_rebut')]
 
    with open ('jobs_%s.sh' % experiment_name, 'w') as rsh:
        for line in cmds:
            rsh.write(line)
            rsh.write('\n')

    scores = dfs.groupby(['lrs', 'datasets', 'model names'], as_index=False).min()  # skipna=True)
    ttas = dfs.groupby(['lrs', 'datasets', 'model names'], as_index=False).mean()  # skipna=True)
