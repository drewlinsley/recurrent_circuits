...
def main():
    ...
    file_path = '/media/data_cifs/cluster_projects/refactor_gammanet'
    file_name =  sys.argv[1]
    h = int(sys.argv[2])
    w = int(sys.argv[3])
    save = sys.argv[4] == 'True'
    out_data = np.load(os.path.join(file_path, file_name))

    out_data_arr = out_data['test_dict'].copy()
    # import ipdb;ipdb.set_trace()
    meta_arr = np.reshape(np.load('/media/data_cifs/tilt_illusion/test/metadata/1.npy'), [-1, 11])
