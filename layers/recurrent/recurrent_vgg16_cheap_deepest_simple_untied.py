...
class Vgg16():
    def __init__(
        ...
        self.image_resize = tf.image.resize_bilinear
        ...
        self.force_alpha_divisive = False
    ...
    def fgru_ops(self, ff_drive, h2, layer_id, i0):
        """Drop-in untied fgru."""
        if layer_id > 3:
            k = (3, 3)
        else:
            k = (1, 1)
        h = tf.layers.conv2d(
            inputs=h2, filters=ff_drive.get_shape().as_list()[-1], kernel_size=k, padding='same', name='%s_%s' % (layer_id, i0), activation=tf.nn.relu)
        return h, h + ff_drive
